<?php
	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		$returnDetailsReturnDate = isset($_POST['returnDetailsReturnDate']);  // This will check to see if 'returnDetailsReturnDate' exists in the $_POST array before attempting to use it.
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsCustomerID)){
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					$updateSaleDetailsSql = 'UPDATE sale SET itemNumber = :itemNumber, saleDate = :saleDate, returnDate = :returnDate, itemName = :itemName, quantity = :quantity, customerName = :customerName, customerID = :customerID WHERE saleID = :saleID';
					$updateSaleDetailsStatement = $conn->prepare($updateSaleDetailsSql);
					$updateSaleDetailsStatement->execute(['itemNumber' => $saleDetailsItemNumber, 'saleDate' => $saleDetailsSaleDate, 'returnDate' => $returnDetailsReturnDate,'itemName' => $saleDetailsItemName, 'quantity' => $saleDetailsQuantity, 'customerName' => $saleDetailsCustomerName, 'customerID' => $customerID, 'saleID' => $saleDetailsSaleID]);
