<?php
	...
	if(isset($_POST['saleDetailsItemNumber'])){
		...
		$returnDate = isset($_POST['returnDetailsReturnDate']) ? htmlentities($_POST['returnDetailsReturnDate']) : ''; // Check if the key exists
		
		// Check if mandatory fields 	 not empty
			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						...
						$insertSaleSql = 'INSERT INTO sale(itemNumber, itemName, quantity, customerID, customerName, saleDate, returnDate) VALUES(:itemNumber, :itemName, :quantity, :customerID, :customerName, :saleDate, :returnDate)';
						$insertSaleStatement = $conn->prepare($insertSaleSql);
						$insertSaleStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'quantity' => $quantity, 'customerID' => $customerID, 'customerName' => $customerName, 'saleDate' => $saleDate, 'returnDate' => $returnDate]);
